...
from .workflow_types import RefillState, ConversationContext, ToolResult
...
class StateDefinition:
    ...
    state: RefillState
    description: str
    entry_actions: List[Callable[[ConversationContext], ConversationContext]] = field(default_factory=list)
    exit_actions: List[Callable[[ConversationContext], ConversationContext]] = field(default_factory=list)
...
class RefillStateMachine:
    ...
    def _initialize_states(self):
        ...
        self.states = {
            ...
            RefillState.IDENTIFY_MEDICATION: StateDefinition(
                ...
                required_tools=["patient_medication_history", "rxnorm_medication_lookup"],
            ...
            RefillState.CONFIRM_DOSAGE: StateDefinition(
                ...
                required_tools=["verify_medication_dosage", "check_drug_interactions", "patient_allergies"],
            ...
            RefillState.CHECK_AUTHORIZATION: StateDefinition(
                ...
                required_tools=["insurance_formulary_check", "prior_authorization_lookup"],
            ...
            RefillState.SELECT_PHARMACY: StateDefinition(
                ...
                required_tools=["find_nearby_pharmacies", "check_pharmacy_inventory", "goodrx_price_lookup"],
    ...
    def _initialize_transitions(self):
        ...
        self.transitions: List[StateTransition] = [
            ...
            StateTransition(
                RefillState.IDENTIFY_MEDICATION, RefillState.CLARIFY_MEDICATION,
                trigger="ambiguous_medication",
                condition=lambda ctx: ctx.medication is None or ctx.medication.get("ambiguous", False),
                description="Medication name is ambiguous or unclear"
            ),
            StateTransition(
                RefillState.IDENTIFY_MEDICATION, RefillState.CONFIRM_DOSAGE,
                trigger="medication_identified",
                condition=lambda ctx: ctx.medication is not None and not ctx.medication.get("ambiguous", False),
                description="Medication successfully identified"
            ),
            StateTransition(
                RefillState.IDENTIFY_MEDICATION, RefillState.ERROR,
            ...
            StateTransition(
                RefillState.CLARIFY_MEDICATION, RefillState.CONFIRM_DOSAGE,
                trigger="medication_clarified",
                condition=lambda ctx: ctx.medication is not None and not ctx.medication.get("ambiguous", False),
                description="Medication successfully clarified"
            ),
            StateTransition(
                RefillState.CLARIFY_MEDICATION, RefillState.ERROR,
            ...
            # From CONFIRM_DOSAGE
            StateTransition(
                RefillState.CONFIRM_DOSAGE, RefillState.CHECK_AUTHORIZATION,
                trigger="dosage_confirmed",
                condition=lambda ctx: ctx.dosage is not None and not (ctx.medication and ctx.medication.get("safety_issues", False)),
            ...
            StateTransition(
                RefillState.CHECK_AUTHORIZATION, RefillState.SELECT_PHARMACY,
                trigger="authorized",
                condition=lambda ctx: not ctx.insurance_info.get("prior_auth_required", False) if ctx.insurance_info else True,
                description="Medication authorized by insurance"
            ),
            StateTransition(
                RefillState.CHECK_AUTHORIZATION, RefillState.ESCALATE_PA,
                trigger="prior_auth_required",
                condition=lambda ctx: ctx.insurance_info.get("prior_auth_required", False) if ctx.insurance_info else False,
                description="Prior authorization required"
            ),
            
            # From SELECT_PHARMACY
            StateTransition(
                RefillState.SELECT_PHARMACY, RefillState.CONFIRM_ORDER,
                trigger="pharmacy_selected",
                condition=lambda ctx: ctx.pharmacy is not None,
                description="Pharmacy selected by patient"
            ),
            StateTransition(
                RefillState.SELECT_PHARMACY, RefillState.ERROR,
            ...
            StateTransition(
                RefillState.CONFIRM_ORDER, RefillState.SELECT_PHARMACY,
                trigger="change_pharmacy",
                description="Patient wants to change pharmacy selection"
            ),
            StateTransition(
                RefillState.CONFIRM_ORDER, RefillState.ERROR,
            ...
            # From ESCALATE_PA
            StateTransition(
                RefillState.ESCALATE_PA, RefillState.SELECT_PHARMACY,
                trigger="pa_approved",
                condition=lambda ctx: ctx.insurance_info.get("pa_status") == "approved" if ctx.insurance_info else False,
                description="Prior authorization approved"
            ),
            StateTransition(
                RefillState.ESCALATE_PA, RefillState.ERROR,
                trigger="pa_denied",
                condition=lambda ctx: ctx.insurance_info.get("pa_status") == "denied" if ctx.insurance_info else False,
                description="Prior authorization denied"
            ),
            
            # From ERROR - Recovery paths
            StateTransition(
                RefillState.ERROR, RefillState.START,
                trigger="restart_conversation",
                description="Start over from the beginning"
            ),
            StateTransition(
                RefillState.ERROR, RefillState.IDENTIFY_MEDICATION,
                trigger="retry_medication",
                description="Try medication identification again"
            ),
            StateTransition(
                RefillState.ERROR, RefillState.CLARIFY_MEDICATION,
    ...
    def _validate_state_machine(self):
        ...
        for transition in self.transitions:
            if transition.from_state not in self.states:
                issues.append(f"Transition from undefined state: {transition.from_state}")
    ...
    def create_session(self, session_id: str, patient_id: str = "12345") -> ConversationContext:
        ...
        self.state_history[session_id] = [(RefillState.START, datetime.now(), "Session created")]
    ...
    def transition(self, session_id: str, trigger: str, **kwargs) -> Tuple[bool, Optional[ConversationContext], Optional[str]]:
        """
        ...
        if trigger not in self.transition_map[current_state]:
            ...
            return False, context, f"Invalid trigger '{trigger}' from {current_state}. Available: {available_triggers}"
        ...
        if not transition.can_transition(context):
            return False, context, f"Transition condition failed for {trigger} from {current_state}"
        ...
        self.logger.info(f"Session {session_id}: {current_state} -> {transition.to_state} via '{trigger}'")
    ...
    def get_state_history(self, session_id: str) -> List[Tuple[RefillState, datetime, str]]:
    ...
    def get_workflow_diagram(self) -> str:
        ...
        for state in RefillState:
            ...
            lines.append(f"    {state.value}{shape}{state.value.replace('_', ' ').title()}{end_shape}")
        
        # Add transitions
        for transition in self.transitions:
            lines.append(f"    {transition.from_state.value} -->|{transition.trigger}| {transition.to_state.value}")
