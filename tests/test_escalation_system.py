...
class TestEscalationSystem:
    ...
    async def test_controlled_substance_escalation(self):
        ...
        response = await manager.process_message(
            "test_controlled",
            "I need to refill my lorazepam"
        )
        
        response_lower = response.message.lower()
        assert any(keyword in response_lower for keyword in [
            "escalat", "pharmacist", "doctor", "controlled"
    ...
    async def test_unknown_medication_escalation(self):
        ...
        response = await manager.process_message(
            "test_unknown",
            "I need to refill my xyz123medicine"
        )
        
        response_lower = response.message.lower()
        # Check if the system properly handles unknown medication (either suggests alternatives or escalates)
        unknown_indicators = [
            "not recognized", "not found", "double-check", "correct name", 
            "suggestions", "common medications", "database", "escalat", "pharmacist", 
            "couldn't find", "no record", "consult"
        ]
        assert any(indicator in response_lower for indicator in unknown_indicators), \
