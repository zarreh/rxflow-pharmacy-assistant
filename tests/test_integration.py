...
from rxflow.workflow.conversation_manager import ConversationManager, ConversationResponse
...
class IntegrationTestSuite:
    ...
    async def run_conversation_flow(self, 
                                  ...
                                  test_name: str,
                                  patient_id: str = "12345") -> Dict[str, Any]:
        """
        ...
        try:
            for i, message in enumerate(messages):
                ...
                state_transitions.append({
                    "step": i + 1,
                    "expected_state": expected_states[i].value if i < len(expected_states) else "unknown",
                    "actual_state": result.current_state.value,
                    "match": result.current_state == expected_states[i] if i < len(expected_states) else False
                })
...
async def run_comprehensive_integration_tests() -> Dict[str, Any]:
    ...
    average_state_accuracy = sum(result.get("state_accuracy", 0) for result in test_results) / total_tests
    ...
    logger.info(f"[INTEGRATION TESTING] Test suite completed - Success rate: {report['summary']['success_rate']:.1%}")
...
def generate_test_recommendations(test_results: List[Dict[str, Any]]) -> List[str]:
    ...
    if failed_tests:
        recommendations.append(f"Review and fix {len(failed_tests)} failed test scenarios")
    
    # Check state accuracy
    low_accuracy_tests = [
        result for result in test_results 
        if result.get("state_accuracy", 1.0) < 0.8
    ]
    ...
    zero_tool_tests = [
        result for result in test_results 
        if result.get("total_tools_used", 0) == 0
    ]
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        print(f"Average State Accuracy: {report['summary']['average_state_accuracy']:.1%}")
        print("\nRecommendations:")
